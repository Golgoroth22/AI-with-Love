...
WEBPAGES_DIR = '/var/www/html/webpages'
WEBPAGE_BASE_URL = 'http://148.253.209.151/webpages'
...
class MCPServerHandler(BaseHTTPRequestHandler):
    ...
    def handle_tools_call(self, request_id, params):
        ...
        try:
            ...
            elif tool_name == 'create_webpage':
                result = self.tool_create_webpage(arguments)
            else:
    ...
    def tool_create_webpage(self, args):
        """
        Create an HTML webpage with provided text

        Arguments:
            text: str - The text content to display on the webpage
            title: str (optional) - Page title (defaults to first 50 chars of text)
        ...
        try:
            ...
            title = args.get('title', text[:50] + ('...' if len(text) > 50 else ''))
            title_escaped = html.escape(title)

            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            unique_id = str(uuid.uuid4())[:8]
            filename = f"page_{timestamp}_{unique_id}.html"
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            url = f"{WEBPAGE_BASE_URL}/{filename}"

            self.log(f"✨ Created webpage: {url}")
            ...
            self.log(f"❌ Failed to create webpage: {str(e)}")
            return {
                'success': False,
                'error': str(e),
                'message': 'Failed to create webpage'
            }
