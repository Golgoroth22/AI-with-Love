...
**Flow Diagram:**
```
User Message → Keyword Detection → Tool Selection
     ↓
...
Card(
    ...
    Row {
        Icon(Icons.Default.Tune, "Настройки поиска")
        Text("Порог схожести: ${(threshold * 100).toInt()}%")
    }
...
if (compareMode) {
    ...
    Column {
        Text("Без фильтра (${unfilteredCount} документов):")
        unfilteredResults.forEach { doc ->
            DocumentCard(doc, highlighted = false)
        }

        Text("С порогом ${threshold} (${filteredCount} документов):")
...
- **Green**: Similarity ≥ 80% (high relevance)
- **Orange**: Similarity ≥ 60% (medium relevance)
...
// Prompt tokens (📤 icon)
Row {
    Text("📤 ${message.promptTokens}")
}

// Completion tokens (📥 icon)
Row {
    Text("📥 ${message.completionTokens}")
}
```
...
private suspend fun compressDialog() {
    ...
    addMessage(Message(
        content = "🗜️ Сжимаю историю диалога...",
        role = Role.SYSTEM
    ))
    ...
    _messages.value = listOf(
        Message(
            content = "📝 История сжата. Краткое содержание:\n\n$summary",
            role = Role.SYSTEM,
            isSummary = true
        )
    )
...
Icon(
    ...
    contentDescription = "Отправить сообщение" // Russian: Send message
)

Button(
    onClick = { /* ... */ },
    modifier = Modifier.semantics {
        contentDescription = "Очистить чат" // Russian: Clear chat
    }
) {
    Text("Очистить")
}
...
3. Check logs for "🗜️ Сжимаю историю диалога..." message
